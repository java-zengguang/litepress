...
public class RoleDao {
    public List<Role> getRoleList(Role role) throws Exception {
        ...
        List list= JDBCUtils.select(sql,Role.class);
        return list;
    }

    public int insertRole(Role role) throws SQLException, IllegalAccessException {
        List list=new ArrayList();
        list.add(role);
        JDBCUtils.insertTables(list,Role.class,"cn_role");
        return 1;
    }


    public int editRole(Role role) throws NoSuchFieldException, IllegalAccessException, SQLException {
