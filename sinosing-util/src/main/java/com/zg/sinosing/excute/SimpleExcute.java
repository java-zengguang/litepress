...
public class SimpleExcute implements ExcuteSQL{
    ...
    @Override
    public List<SinoSingSQLLogEntity> excute(List<SinoSingSQLLogEntity> list) throws SQLException {

        for (SinoSingSQLLogEntity sinoSingSQLLogEntity:list){
            if("3".equals(sinoSingSQLLogEntity.executestate)) {
                ArrayList sqlList = new ArrayList();
                String dataSourceName = sinoSingSQLLogEntity.getDataSource();
                String sql = sinoSingSQLLogEntity.devsql;
                if (sql.contains(";")) {
                ...
                if (jdbcUtil == null) {
                    ...
                    sinoSingSQLLogEntity.setErrormassage("数据库链接获取失败");
                }
                try {
                    ...
                    sinoSingSQLLogEntity.executestate = "-1";
                    sinoSingSQLLogEntity.errormassage = sinoSingSQLLogEntity.errormassage + throwables.getMessage();
                    continue;
                } finally {
                    jdbcUtil.commit();
