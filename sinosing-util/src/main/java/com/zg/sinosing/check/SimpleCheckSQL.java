...
public class SimpleCheckSQL implements CheckSQL {
    ...
    public boolean checkSQLScript(SinoSingSQLLogEntity sinoSingSQLLogEntity) {

        String vsql = sinoSingSQLLogEntity.basesql;
        if (vsql == null || vsql.equals("")) {
            sinoSingSQLLogEntity.setErrormassage("检查未通过:脚本不能为空！");
            return false;
        }
        ...
        if (true) {
            ...
            if (sql == null || sql.equals("")) {
                sinoSingSQLLogEntity.setErrormassage("  检查未通过:不能使用外键，建议使用业务逻辑判断来代替外键!!");
                return false;
            }
        ...
        if (sql.contains("database") || sql.contains("revoke") || sql.contains("flush") || sql.contains("truncate") || sql.contains("rename") || sql.contains("view") || sql.contains("function") || sql.contains("procedure") || sql.contains("drop")) {
            sinoSingSQLLogEntity.setErrormassage("请注意:包含数据库敏感关键字(database,revoke,flush,truncate,rename,view,function,procedure,drop)中的一个或多个，为防止误操作，请人工检查!!");
         //   return false;
        }

            if (sql.contains("sequence")) {
                sinoSingSQLLogEntity.setErrormassage("检查未通过:创建序列请注意!!");
                return false;
            }

            if (sql.contains("create table") && !(sql.contains("primary") && sql.contains("key"))) {
                sinoSingSQLLogEntity.setErrormassage("检查未通过:新创建表没有主键，表必须有主键!!");
                return false;
            }


            if (!sql.endsWith(";")) {
                sinoSingSQLLogEntity.setErrormassage("检查未通过:脚本必须有结束符!!");
                return false;
            }
            ...
            if (true) {
                ...
                for (int i = 0; i < sqlArray.length - 1; i++) {
                    if (sqlArray[i].trim().length() < 1) {
                        sinoSingSQLLogEntity.setErrormassage("检查未通过:脚本分割检查有误，请检查是否多了结算符!!");
                ...
                if (list.contains("create") && !list.contains("synonym") || list.contains("alter")) {
                    ...
                    if ("table".equals(opreateObject)) {
                        ...
                        if ("".equals(flag) && "alter".equals(opreate) && !sql.contains("primary")) {
                            sinoSingSQLLogEntity.setErrormassage("检查未通过:生产没有这个表 " + "tablename:" + tableName);
                            return false;
                        }
                        if ("1".equals(flag) && "create".equals(opreate)) {
                            sinoSingSQLLogEntity.setErrormassage("检查未通过:生产已经有这个表了 " + "tablename:" + tableName);
                            return false;
                        }

                        if (("alter").equals(opreate)) {
                            if (("drop").equals(exeOpreate)) {
                                sinoSingSQLLogEntity.setErrormassage("检查未通过:单个脚本不允许 drop ");
                                return false;
                            }
                            if (("rename").equals(exeOpreate)) {
                                sinoSingSQLLogEntity.setErrormassage("检查未通过:单个脚本不允许 rename ");
                                return false;
                            }
                            if ("add".equals(exeOpreate)) {
                                String flag1 = JDBCUtils.getOneValue("select 1 from databasetablestructure d where d.environment  in ('pro') and d.databasename='"+sinoSingSQLLogEntity.databasename+"' and d.tablename ='" + tableName + "' and d.columnname ='" + columnName + "' ");
                                if ("1".equals(flag1)) {
                                    sinoSingSQLLogEntity.setErrormassage("检查未通过:生产已有此字段" + "tablename:" + tableName + " columnname:" + columnName);
                                    return false;
                                }
                                ...
                                if ("1".equals(flag1)) {
                                    ...
                                    sinoSingSQLLogEntity.setErrormassage("检查未通过:老核心已有此字段，且字段类型或长度不一致" + "tablename:" + tableName + " columnname:" + columnName);
                                   // return false;
                                }
                            ...
                            if (("modify").equals(exeOpreate)) {
                                ...
                                if ("".equals(oldColumnType)) {
                                    sinoSingSQLLogEntity.setErrormassage("检查未通过:生产不存在此字段" + "tablename:" + tableName + " columnname:" + columnName);
                                    return false;
                                }
                                if (!compareColumnType(oldColumnType, columnType, sinoSingSQLLogEntity)) {
                                    sinoSingSQLLogEntity.setErrormassage("检查未通过:字段类型校验失败" + "tablename:" + tableName + " columnname:" + columnName);
                                    return false;
                                }
                            }
    ...
    private boolean compareColumnType(String oldColumnType, String columnType, SinoSingSQLLogEntity sinoSingSQLLogEntity) {
        ...
        if (!oldColumnFlag.equals(columnFlag)) {
            sinoSingSQLLogEntity.setErrormassage("检查未通过:脚本中涉及字段类型变更！");
            return false;
        }
        ...
        if (oldColumnLengthStrs != null && columnLengthStrs != null) {
            ...
            if (oldColumnLengthStrs.length != columnLengthStrs.length) {
                sinoSingSQLLogEntity.setErrormassage("检查未通过:脚本字段类型涉及精度变更！");
                return false;
            }
            ...
            for (int i = 0; i < columnLengthStrs.length; i++) {
                ...
                if (columnLength < oldColumnLength) {
                    sinoSingSQLLogEntity.setErrormassage("检查未通过:脚本字段长度不允许改小！");
                    return false;
                }
    ...
    //检查一个对象的脚本，主要做合法性检查
    private boolean checkOneEntitySQL(SinoSingSQLLogEntity sinoSingSQLLogEntity) {
        if (checkSQLScript(sinoSingSQLLogEntity)) {
            sinoSingSQLLogEntity.executestate = "3";
            return true;
        } else {
            sinoSingSQLLogEntity.executestate = "-1";
            return false;
        }

    }

    private List<String> getInitSQL(SinoSingSQLLogEntity sinoSingSQLLogEntity) throws Exception {

        List<String> list = createTableSQL(sinoSingSQLLogEntity);
        return list;
    }
    ...
    private List<String> createTableSQL(SinoSingSQLLogEntity sinoSingSQLLogEntity) throws Exception {
        List<String> list = new ArrayList<>();

        Set<String> tableNames = getTableFromSQL(sinoSingSQLLogEntity.getBasesql());
        for (String tableName : tableNames) {
            String sql = "";
            List<DatabaseTableStructureEntity> databaseTableStructureEntities = strcutureService.getTableStructure(sinoSingSQLLogEntity.environment, sinoSingSQLLogEntity.databasename, tableName);
    ...
    private boolean initTestEnvironment(List<SinoSingSQLLogEntity> list){

        NewJDBCUtil jdbcUtil = new NewJDBCUtil("test");
        try {
            List<SinoSingSQLLogEntity> testEntitys = new ArrayList<>();
            //解析脚本，获取操作的表，检验套表规范
...
/*            initSQLList.add(" CREATE TABLE `ldcode` (\n" +
            ...
            for (SinoSingSQLLogEntity sinoSingSQLLogEntity : list) {
                //模拟环境复制对象
                SinoSingSQLLogEntity testEntity = (SinoSingSQLLogEntity) sinoSingSQLLogEntity.clone();
                testEntity.environment = "test";
                testEntitys.add(testEntity);
                //拉长字段和增加字段，需要在目标环境建表
                if ("增加字段".equals(sinoSingSQLLogEntity.sqlpurpose) || "拉长字段".equals(sinoSingSQLLogEntity.sqlpurpose)) {
                    List<String> sqlList = getInitSQL(sinoSingSQLLogEntity);
    ...
    private boolean verificationCheck(List<SinoSingSQLLogEntity> list) throws Exception {
        String errorMessage="";
        String stageFlag="3";
        ...
        if(!initTestEnvironment(list)){
            ...
            NewJDBCUtil jdbcUtil=new NewJDBCUtil("test");
            for (SinoSingSQLLogEntity sinoSingSQLLogEntity : list) {
                List<String> executeSQLList = formatSQL(sinoSingSQLLogEntity.basesql);
                try {
                    ...
                    sinoSingSQLLogEntity.setErrormassage("脚本执行错误"+message);
                    continue;
                }
            }
...
/*        checkResult=strcutureService.getCompareResult("test","CTT");
        ...
        for(SinoSingSQLLogEntity sinoSingSQLLogEntity:list){
            sinoSingSQLLogEntity.setErrormassage(errorMessage);
            sinoSingSQLLogEntity.executestate=stageFlag;
        }
        //回滚环境，删除数据库
        List<String> initSQLList = new ArrayList();
        NewJDBCUtil jdbcUtil=new NewJDBCUtil("test");
    ...
    @Override
    public List<SinoSingSQLLogEntity> checkSQL(List<SinoSingSQLLogEntity> list) throws Exception {
        //分组+单个校验
        Map<String, List<SinoSingSQLLogEntity>> ddlMapList = new HashMap<>();
        for (SinoSingSQLLogEntity sinoSingSQLLogEntity : list) {
            //初步校验失败的不进入分组校验
            if (checkOneEntitySQL(sinoSingSQLLogEntity) == false) {
                continue;
            }else {
                //只有ddl语句做分组校验
                if ("DDL".equals(sinoSingSQLLogEntity.sqltype.toUpperCase())) {
                    String batchNo = sinoSingSQLLogEntity.batchno;
                    List<SinoSingSQLLogEntity> sqlLogEntities = ddlMapList.get(batchNo);
                    if (sqlLogEntities == null) {
                        sqlLogEntities = new ArrayList<>();
                    }
                    sqlLogEntities.add(sinoSingSQLLogEntity);
        ...
        for (String key : keySet) {
            List<SinoSingSQLLogEntity> sqlLogEntities = ddlMapList.get(key);
