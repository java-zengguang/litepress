...
public class JDBCUtil {
    ...
    public JDBCUtil(OptionDB entity) {
        try {
            setConnection(entity);
        } catch (Exception e) {
            e.printStackTrace();
            return;
        }
    ...
    public void setConnection(OptionDB entity) {

        try {
            Class.forName(entity.driver);
            conn = DriverManager.getConnection(entity.url, entity.username, entity.password);
